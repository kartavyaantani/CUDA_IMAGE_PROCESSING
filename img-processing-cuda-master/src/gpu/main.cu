...
int main(int argc, char** argv)
{
    ...
    cv::Mat image;
    image = cv::imread(argv[1], cv::IMREAD_UNCHANGED);  // Updated
    if (!image.data)
    {
    ...
    else if (func_name == "edge_detect")
    {
        ...
        cv::Mat dst;
        double otsu_threshold = cv::threshold(grey_img, dst, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);  // Updated
    ...
    cv::namedWindow("Display Window", cv::WINDOW_AUTOSIZE);  // Updated
