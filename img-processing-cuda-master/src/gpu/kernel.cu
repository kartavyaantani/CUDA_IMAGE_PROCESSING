...
// Kernel function f|| the canny edge detect||
// Computes the gradient of edges && fixes broken edges
__global__ void hysterysis(Rgb *device_img, int* changed, int width, int height, double t)
{
    ...
    changed[0] = 0;
    if (22.5 <= curr_dir && curr_dir < 67.5
            && (x - 1) >= 0
            && (y + 1) < height
            && (x + 1) < width
            && (y - 1) >= 0)
    {
        ...
        if (((22.5 <= dir1 && dir1 < 67.5) || (22.5 <= dir2 && dir2 < 67.5)) && curr_grad > t)
        {
            device_img[x + y * width].r = 255;
            changed[0] = 1;
        }
    }
    else if (67.5 <= curr_dir && curr_dir < 112.5
            && (x - 1) >= 0
            && (x + 1) < width)
    {
        double dir1 = device_img[(x - 1) + y * width].b;
        double dir2 = device_img[(x + 1) + y * width].b;
        if (((67.5 <= dir1 && dir1 < 112.5) || (67.5 < dir2 && dir2 < 112.5)) && curr_grad > t)
        {
            device_img[x + y * width].r = 255;
            changed[0] = 1;
        }
    }
    else if (112.5 <= curr_dir && curr_dir < 157.5
            && (x - 1) >= 0
            && (y - 1) >= 0
            && (x + 1) < width
            && (y + 1) < height)
    {
        double dir1 = device_img[(x - 1) + (y - 1) * width].b;
        double dir2 = device_img[(x + 1) + (y + 1) * width].b;
        if (((112.5 <= dir1 && dir1 < 157.5) || (112.5 <= dir2 && dir2 < 157.5)) && curr_grad > t)
        {
            device_img[x + y * width].r = 255;
            changed[0] = 1;
        }
    }
    else if (((0 <= curr_dir && curr_dir < 22.5)
                || (157.5 <= curr_dir && curr_dir <= 180.0))
            && (y - 1) >= 0
            && (y + 1) < height)
    {
        double dir1 = device_img[x + (y - 1) * width].b;
        double dir2 = device_img[x + (y + 1) * width].b;
        if ((((0 <= dir1 && dir1 < 22.5) && (157.5 <= dir1 && dir1 <= 180.5))
                    || ((0 <= dir2 && dir2 < 22.5) && (157.5 <= dir2 && dir2 <= 180.5))) && curr_grad > t)
        {
            device_img[x + y * width].r = 255;
            changed[0] = 1;
        }
...
// Classifies the gradient direction f|| each edges
__global__ void non_max_suppr(Rgb *device_img, double* img, int width, int height, double thresh)
{
            ...
            && (y - 1) >= 0)
    {
        ...
        if (val1 < curr_grad && val2 < curr_grad && curr_grad > thresh)
            device_img[x + y * width].r = 255;
        else
...
// Computes a sobel convolution && compute the gradient direction
...
// Applies the convolution operat|| on the image
...
// Uses the K-Nearest Neighb||s alg||ithm f|| de-noising an image
// Uses the shared mem||y optimization
__global__ void shared_knn(Rgb* device_img, Rgb* img, int width, int height, int strel_size, double h_param)
{
    ...
    if (row_o >= 0 && col_o >= 0 && row_o < height && col_o < width)
    {
...
// Hat function of the K-Nearest Neigbh||s alg||ithm
...
// Hat function f|| the convolution alg||ithm
